...
export class Explorer {
	...
	populateMainInstances() {
		const coordinateCache = new CoordinateCache(this.game.getGrid().length);
		let nextInstances = explore(this.game, coordinateCache);
		...
		for (let moveHash of nextInstances.keys().toArray()) {
			...
			let furtherInstances = explore(nextMainInstance, coordinateCache);
	...
	public terminate() {
		...
		this.instances.clear(); // Vide la map des instances
		this.mainInstances = [];
		this.bestMoves = [];
	}
	...
	rateInstance(instance: SimpleGameInstance): ScoredGameInstance {
		const coordinateCache = new CoordinateCache(instance.getGrid().length);
		return new ScoredGameInstance(instance.getGrid(), instance.getTurn(), instance.moves, this.heuristic(instance, coordinateCache));
	}
}

type CheckableHexagon = [Coordinate, number, number]; // [[y, x], cout, nombre de ponts]

export function basicHeuristic(instance: SimpleGameInstance, cache: CoordinateCache): Score {
	let grid = instance.getGrid();
	let player1Score = attributeScore(grid, 1, cache); // On calcule le score du joueur 1
	let player2Score = attributeScore(grid, 2, cache); // On calcule le score du joueur 2
	//
	if (player1Score[0] === 0) {
...
function explore(game: ScoredGameInstance, cache: CoordinateCache): Map<number, ScoredGameInstance> {
	...
	for (let move of playableMoves) {
		...
		const scoredNewGame = newGame.giveInitialScore(basicHeuristic(newGame, cache));
...
export function attributeScore(grid: number[][], player: number, cache: CoordinateCache): [number, number] {
	...
	if (player === 1) { 
		for (let i = 0; i < grid.length; i++) {
			if (grid[i][0] === 1) {
				hexagonsToCheck.push([cache.get(i, 0), 0, 0]);
			}
			else if (grid[i][0] === 0) {
				if (i > 0 && grid[i - 1][1] === 1 && grid[i - 1][0] === 0) {
					hexagonsToCheck.push([cache.get(i, 0), 0, 1]);
				} else if (i < grid.length - 1 && grid[i][1] === 1 && grid[i + 1][0] === 0) {
					hexagonsToCheck.push([cache.get(i, 0), 0, 1]);
				} else {
					hexagonsToCheck.push([cache.get(i, 0), 1, 0]);
				}
			}
		}
	}
		...
		for (let i = 0; i < grid.length; i++) {
			if (grid[0][i] === 2) {
				hexagonsToCheck.push([cache.get(0, i), 0, 0]);
			}
			else if (grid[0][i] === 0) {
				if (i > 0 && grid[1][i - 1] === 2 && grid[0][i - 1] === 0) {
					hexagonsToCheck.push([cache.get(0, i), 0, 1]);
				} else if (i < grid.length - 1 && grid[1][i] === 2 && grid[0][i + 1] === 0) {
					hexagonsToCheck.push([cache.get(0, i), 0, 1]);
				} else {
					hexagonsToCheck.push([cache.get(0, i), 1, 0]);
				}
			}
		}
	}
	...
	while (hexagonsToCheck.length > 0) {
		...
		let surroundingHexagons = getSurroundingHexagons(grid.length, hexagon[0][0], hexagon[0][1], cache); // Récupère les hexagones environnants
			...
			if (!isCoordinateInArray(checkedHexagons, surroundingHexagon)) {
				...
				if (grid[surroundingHexagon[0]][surroundingHexagon[1]] === player) { // Si la case suivante est occupée par le joueur actuel alors pas d'ajout de cout
					...
					if (boundCheck === size - 1) { // Si la case suivante est sur la dernière colonne, on a un pont assuré
						...
						let candidates = getOuterBridgeHexagons(hexagon[0], surroundingHexagon, size, cache).filter(coord => grid[coord[0]][coord[1]] === player); // On cherche les hexagones extérieurs formant un potientiel pont
						if (candidates.length > 0) {
							candidates = candidates.filter(coord => getInnerBridgeHexagons(hexagon[0], coord, cache).every(coord2 => grid[coord2[0]][coord2[1]] === 0)); // On vérifie si les hexagones intérieurs sont libres
							if (candidates.length > 0) {
				...
				if (boundCheck === size - 1) {// Il existe une erreur de logique, des cases d'arrivée sont considérées comme des ponts, mais elles ne le sont pas
					...
					let existingHexagon = hexagonsToCheck.find(hex => hex[0] == surroundingHexagon);
					if (existingHexagon) {
						if (existingHexagon[1] > cost) {
							existingHexagon[1] = cost; // Met à jour le cout de l'hexagone si le nouveau cout est plus faible
							existingHexagon[2] = bridges
						} else if (existingHexagon[1] === cost && existingHexagon[2] > hexagon[2]) {
							existingHexagon[2] = bridges
						}
					} else {
						let newHexagonIndex = sortedIndex(hexagonsToCheck, [surroundingHexagon, cost, bridges]);
						hexagonsToCheck.splice(newHexagonIndex, 0, [surroundingHexagon, cost, bridges]); // Ajoute l'hexagone à la liste des hexagones à vérifier
					}
				}
			}
		}
...
function isCoordinateInArray(array: Array<Coordinate>, coord: Coordinate) {
	...
	for (let i = 0; i < array.length; i++) {
		if (array[i][0] === coord[0] && array[i][1] === coord[1]) {
			if (array.includes(coord) == false) {
				throw new Error("Array.includes() fake false");
			}
			return true;
		}
	}
	if (array.includes(coord) == true) {
		throw new Error("Array.includes() fake true");
	}
	return false;
	*/
	return array.includes(coord); // Only valid if all objects are part of the same CoordinateCache
}

function getInnerBridgeHexagons(hex1: Coordinate, hex2: Coordinate, cache: CoordinateCache): Array<Coordinate> {
	...
	//
	if (diffX === 1 && diffY === 1) {
		return [cache.get(y, x - 1), cache.get(y - 1, x)];
	}
	if (diffX === -1 && diffY === 2) {
		return [cache.get(y - 1, x), cache.get(y - 1, x + 1)];
	}
	if (diffX === -2 && diffY === 1) {
		return [cache.get(y - 1, x + 1), cache.get(y, x + 1)];
	}
	if (diffX === -1 && diffY === -1) {
		return [cache.get(y, x + 1), cache.get(y + 1, x)];
	}
...
function getOuterBridgeHexagons(outer: Coordinate, inner: Coordinate, size: number, cache: CoordinateCache): Array<Coordinate> {
	const diffY = outer[0] - inner[0];
	const diffX = outer[1] - inner[1];
	const y = outer[0];
	const x = outer[1];

	const out: Coordinate[] = [];
	// helper to push only in-bounds coordinates (inline bounds check for speed)
	const pushIfValid = (yy: number, xx: number) => {
		if (yy >= 0 && yy < size && xx >= 0 && xx < size) out.push(cache.get(yy, xx));
	};
...
function getSurroundingHexagons(size: number, y: number, x: number, cache: CoordinateCache): Array<Coordinate> {
	let surroundingHexagons: Array<Coordinate> = [];
	if (x > 0) {
		surroundingHexagons.push(cache.get(y, x - 1));
	}
	if (x < size - 1) {
		surroundingHexagons.push(cache.get(y, x + 1));
	}
	if (y > 0) {
		surroundingHexagons.push(cache.get(y - 1, x));
	}
	if (y < size - 1) {
		surroundingHexagons.push(cache.get(y + 1, x));
	}
	if (x > 0 && y < size - 1) {
		surroundingHexagons.push(cache.get(y + 1, x - 1));
	}
	if (x < size - 1 && y > 0) {
		surroundingHexagons.push(cache.get(y - 1, x + 1));
	}
...
function sortedIndex(array: Array<CheckableHexagon>, value: CheckableHexagon): number {
	let low = 0;
	let high = array.length;
	let valueScore = value[1];

	while (low < high) {
		let mid = low + high >>> 1;
		if (array[mid][1] > valueScore) low = mid + 1;
		else high = mid;
	}
	return low;
}